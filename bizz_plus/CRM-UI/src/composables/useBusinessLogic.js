...
const getValidatedFiltersState = (key, defaultValue) => {
    try {
        const stored = localStorage.getItem(key);
        if (!stored)
            return defaultValue;
...
export const useBusinessLogic = () => {
    const selectedEntity = ref(getStoredState('selectedEntity', 'manufacturer'));
    const selectedManufacturer = ref(getStoredState('selectedManufacturer', null));
    const selectedDistributor = ref(getStoredState('selectedDistributor', null));
    const selectedEntityId = ref(getStoredState('selectedEntityId', ''));
    ...
    const filters = reactive(getValidatedFiltersState('filters', defaultFilters));
    const associatedFilters = reactive(getValidatedFiltersState('associatedFilters', defaultFilters));
    const manufacturers = ref(mockManufacturers);
    const distributors = ref(mockDistributors);
    const interactions = ref(mockInteractions);
    const agreement = ref(mockAgreement);
    ...
    const pairedList = computed(() => {
        if (selectedEntity.value === 'manufacturer' && selectedManufacturer.value) {
            return distributors.value.filter(d => d.category === selectedManufacturer.value?.category &&
                d.subCategory === selectedManufacturer.value?.subCategory);
        }
        if (selectedEntity.value === 'distributor' && selectedDistributor.value) {
            return manufacturers.value.filter(m => m.category === selectedDistributor.value?.category &&
    ...
    const updateManufacturerStatus = (manufacturerId, newStatus) => {
    ...
    const clearAssociatedFilters = () => {
        Object.keys(associatedFilters).forEach(key => {
            associatedFilters[key] = [];
        });
        persistState();
    };
    const updateLocationFilters = (type, values, isAssociated = false) => {
    ...
    const updateCategoryFilters = (type, values, isAssociated = false) => {
        const targetFilters = isAssociated ? associatedFilters : filters;
    ...
    const setSelectedDistributor = (distributor) => {
